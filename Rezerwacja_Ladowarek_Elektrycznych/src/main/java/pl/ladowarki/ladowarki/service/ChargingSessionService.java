package pl.ladowarki.ladowarki.service;
...
import pl.ladowarki.ladowarki.repository.ChargingSessionRepository;
import pl.ladowarki.ladowarki.repository.ChargingStationRepository;

@Service
@RequiredArgsConstructor
public class ChargingSessionService {
    private final ChargingStationRepository resourceRepository;
    private final ChargingSessionRepository recordRepository;
    private final AppUserService appUserService;

    public List<ChargingStation> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<ChargingSession> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<ChargingSession> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public ChargingStation findBestResource(ChargingType category, int requiredCapacity,
    ...
    @Transactional
    public ChargingSession createRecord(ChargingType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
        ChargingStation resource = findBestResource(category, requiredCapacity, minQuality,
                taskUnits, priority, startTime, endTime);
        ChargingSession record = new ChargingSession();
    ...
    public BigDecimal calculateScore(ChargingStation resource, int requiredCapacity,
    ...
    public BigDecimal calculatePrice(ChargingStation resource, int taskUnits, int priority) {
    ...
    public int calculateRequiredHours(ChargingStation resource, int taskUnits) {
    ...
    private void validateSearch(ChargingType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
