package pl.ladowarki.ladowarki;
...
import pl.ladowarki.ladowarki.service.AppUserService;
import pl.ladowarki.ladowarki.service.ChargingSessionService;

@ExtendWith(MockitoExtension.class)
class ChargingSessionServiceTest {
    @Mock
    private ChargingStationRepository resourceRepository;

    @Mock
    private ChargingSessionRepository recordRepository;
    ...
    void shouldChooseBestScoredResource() {
        ...
        when(resourceRepository.findAll()).thenReturn(List.of(
                resource(1L, ChargingType.CITY, 10, 5, 5, "100.00"),
                resource(2L, ChargingType.CITY, 3, 3, 5, "70.00"),
                resource(3L, ChargingType.FAST, 3, 3, 5, "50.00")));
        when(recordRepository.existsOverlapping(anyLong(), eq(start), eq(end))).thenReturn(false);

        ChargingStation result = service.findBestResource(ChargingType.CITY,
                3, 3, 10, 1, start, end);
    ...
    void shouldSkipOverlappingBetterResource() {
        ...
        LocalDateTime end = start.plusHours(3);
        when(resourceRepository.findAll()).thenReturn(List.of(
                resource(1L, ChargingType.CITY, 3, 3, 5, "70.00"),
                resource(2L, ChargingType.CITY, 5, 4, 5, "90.00")));
    ...
    void shouldRejectWhenTimeWindowIsTooShort() {
        ...
        assertThrows(RuntimeException.class,
                () -> service.findBestResource(ChargingType.CITY,
                        3, 3, 20, 1, start, end));
    }

    @Test
    void shouldCalculateStartedRequiredHoursAndPrice() {
        ChargingStation resource = resource(1L, ChargingType.CITY,
                3, 3, 5, "100.00");
    ...
    private ChargingStation resource(Long id, ChargingType category, int capacity,
            int quality, int unitsPerHour, String price) {
        ChargingStation resource = new ChargingStation();
        resource.setId(id);
