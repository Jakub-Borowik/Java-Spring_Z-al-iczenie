package pl.fotowoltaika.fotowoltaika.service;
...
public class SolarServiceOrderService {
    private final SolarTechnicianRepository resourceRepository;
    private final SolarServiceOrderRepository recordRepository;
    private final AppUserService appUserService;

    public List<SolarTechnician> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<SolarServiceOrder> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<SolarServiceOrder> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public SolarTechnician findBestResource(SolarServiceType category, int requiredCapacity,
        ...
        return resourceRepository.findAll().stream()
                .filter(SolarTechnician::isAvailable)
    ...
    @Transactional
    public SolarServiceOrder createRecord(SolarServiceType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
        SolarTechnician resource = findBestResource(category, requiredCapacity, minQuality,
                taskUnits, priority, startTime, endTime);
        SolarServiceOrder record = new SolarServiceOrder();
    ...
    public BigDecimal calculateScore(SolarTechnician resource, int requiredCapacity,
    ...
    public BigDecimal calculatePrice(SolarTechnician resource, int taskUnits, int priority) {
    ...
    public int calculateRequiredHours(SolarTechnician resource, int taskUnits) {
    ...
    private void validateSearch(SolarServiceType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
