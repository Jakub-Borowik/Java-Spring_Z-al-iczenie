...
public interface SolarServiceOrderRepository extends JpaRepository<SolarServiceOrder, Long> {
    List<SolarServiceOrder> findByUserId(Long userId);

    @Query("""
            select count(record) > 0 from SolarServiceOrder record
