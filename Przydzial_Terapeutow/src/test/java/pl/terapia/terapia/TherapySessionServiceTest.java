...
import pl.terapia.terapia.service.AppUserService;
import pl.terapia.terapia.service.TherapySessionService;

@ExtendWith(MockitoExtension.class)
class TherapySessionServiceTest {
    @Mock
    private TherapistRepository resourceRepository;

    @Mock
    private TherapySessionRepository recordRepository;
    ...
    void shouldChooseBestScoredResource() {
        ...
        when(resourceRepository.findAll()).thenReturn(List.of(
                resource(1L, TherapyType.INDIVIDUAL, 10, 5, 5, "100.00"),
                resource(2L, TherapyType.INDIVIDUAL, 3, 3, 5, "70.00"),
                resource(3L, TherapyType.COUPLE, 3, 3, 5, "50.00")));
        when(recordRepository.existsOverlapping(anyLong(), eq(start), eq(end))).thenReturn(false);

        Therapist result = service.findBestResource(TherapyType.INDIVIDUAL,
                3, 3, 10, 1, start, end);
    ...
    void shouldSkipOverlappingBetterResource() {
        ...
        LocalDateTime end = start.plusHours(3);
        when(resourceRepository.findAll()).thenReturn(List.of(
                resource(1L, TherapyType.INDIVIDUAL, 3, 3, 5, "70.00"),
                resource(2L, TherapyType.INDIVIDUAL, 5, 4, 5, "90.00")));
    ...
    void shouldRejectWhenTimeWindowIsTooShort() {
        ...
        assertThrows(RuntimeException.class,
                () -> service.findBestResource(TherapyType.INDIVIDUAL,
                        3, 3, 20, 1, start, end));
    }

    @Test
    void shouldCalculateStartedRequiredHoursAndPrice() {
        Therapist resource = resource(1L, TherapyType.INDIVIDUAL,
                3, 3, 5, "100.00");
    ...
    private Therapist resource(Long id, TherapyType category, int capacity,
            int quality, int unitsPerHour, String price) {
        Therapist resource = new Therapist();
        resource.setId(id);
