package pl.terapia.terapia.service;
...
import pl.terapia.terapia.repository.TherapySessionRepository;
import pl.terapia.terapia.repository.TherapistRepository;

@Service
@RequiredArgsConstructor
public class TherapySessionService {
    private final TherapistRepository resourceRepository;
    private final TherapySessionRepository recordRepository;
    private final AppUserService appUserService;

    public List<Therapist> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<TherapySession> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<TherapySession> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public Therapist findBestResource(TherapyType category, int requiredCapacity,
    ...
    @Transactional
    public TherapySession createRecord(TherapyType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
        Therapist resource = findBestResource(category, requiredCapacity, minQuality,
                taskUnits, priority, startTime, endTime);
        TherapySession record = new TherapySession();
    ...
    public BigDecimal calculateScore(Therapist resource, int requiredCapacity,
    ...
    public BigDecimal calculatePrice(Therapist resource, int taskUnits, int priority) {
    ...
    public int calculateRequiredHours(Therapist resource, int taskUnits) {
    ...
    private void validateSearch(TherapyType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
