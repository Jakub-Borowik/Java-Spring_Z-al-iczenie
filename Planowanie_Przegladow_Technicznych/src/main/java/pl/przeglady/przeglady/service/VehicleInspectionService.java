package pl.przeglady.przeglady.service;
...
import pl.przeglady.przeglady.repository.VehicleInspectionRepository;
import pl.przeglady.przeglady.repository.InspectionStationRepository;

@Service
@RequiredArgsConstructor
public class VehicleInspectionService {
    private final InspectionStationRepository resourceRepository;
    private final VehicleInspectionRepository recordRepository;
    private final AppUserService appUserService;

    public List<InspectionStation> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<VehicleInspection> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<VehicleInspection> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public InspectionStation findBestResource(InspectionType category, int requiredCapacity,
        ...
        return resourceRepository.findAll().stream()
                .filter(InspectionStation::isAvailable)
    ...
    @Transactional
    public VehicleInspection createRecord(InspectionType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
        InspectionStation resource = findBestResource(category, requiredCapacity, minQuality,
    ...
    public BigDecimal calculateScore(InspectionStation resource, int requiredCapacity,
    ...
    public BigDecimal calculatePrice(InspectionStation resource, int taskUnits, int priority) {
    ...
    public int calculateRequiredHours(InspectionStation resource, int taskUnits) {
    ...
    private void validateSearch(InspectionType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
