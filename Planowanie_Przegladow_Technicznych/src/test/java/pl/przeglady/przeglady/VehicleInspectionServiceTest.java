package pl.przeglady.przeglady;
...
import pl.przeglady.przeglady.service.AppUserService;
import pl.przeglady.przeglady.service.VehicleInspectionService;

@ExtendWith(MockitoExtension.class)
class VehicleInspectionServiceTest {
    @Mock
    private InspectionStationRepository resourceRepository;

    @Mock
    private VehicleInspectionRepository recordRepository;
    ...
    void shouldChooseBestScoredResource() {
        ...
        when(resourceRepository.findAll()).thenReturn(List.of(
                resource(1L, InspectionType.CAR, 10, 5, 5, "100.00"),
                resource(2L, InspectionType.CAR, 3, 3, 5, "70.00"),
                resource(3L, InspectionType.TRUCK, 3, 3, 5, "50.00")));
        when(recordRepository.existsOverlapping(anyLong(), eq(start), eq(end))).thenReturn(false);

        InspectionStation result = service.findBestResource(InspectionType.CAR,
                3, 3, 10, 1, start, end);
    ...
    void shouldSkipOverlappingBetterResource() {
        ...
        LocalDateTime end = start.plusHours(3);
        when(resourceRepository.findAll()).thenReturn(List.of(
                resource(1L, InspectionType.CAR, 3, 3, 5, "70.00"),
                resource(2L, InspectionType.CAR, 5, 4, 5, "90.00")));
    ...
    void shouldRejectWhenTimeWindowIsTooShort() {
        ...
        assertThrows(RuntimeException.class,
                () -> service.findBestResource(InspectionType.CAR,
                        3, 3, 20, 1, start, end));
    }

    @Test
    void shouldCalculateStartedRequiredHoursAndPrice() {
        InspectionStation resource = resource(1L, InspectionType.CAR,
                3, 3, 5, "100.00");
    ...
    private InspectionStation resource(Long id, InspectionType category, int capacity,
            int quality, int unitsPerHour, String price) {
        InspectionStation resource = new InspectionStation();
        resource.setId(id);
