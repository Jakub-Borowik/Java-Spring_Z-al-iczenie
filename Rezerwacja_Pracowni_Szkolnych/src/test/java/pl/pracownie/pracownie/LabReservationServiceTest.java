package pl.pracownie.pracownie;
...
import pl.pracownie.pracownie.service.AppUserService;
import pl.pracownie.pracownie.service.LabReservationService;

@ExtendWith(MockitoExtension.class)
class LabReservationServiceTest {
    @Mock
    private SchoolLabRepository resourceRepository;

    @Mock
    private LabReservationRepository recordRepository;

    @Mock
    private AppUserService appUserService;

    @InjectMocks
    private LabReservationService service;
    ...
    void shouldChooseBestScoredResource() {
        ...
        when(resourceRepository.findAll()).thenReturn(List.of(
                resource(1L, LabType.CHEMISTRY, 10, 5, 5, "100.00"),
                resource(2L, LabType.CHEMISTRY, 3, 3, 5, "70.00"),
                resource(3L, LabType.COMPUTER, 3, 3, 5, "50.00")));
        when(recordRepository.existsOverlapping(anyLong(), eq(start), eq(end))).thenReturn(false);

        SchoolLab result = service.findBestResource(LabType.CHEMISTRY,
                3, 3, 10, 1, start, end);
    ...
    void shouldSkipOverlappingBetterResource() {
        ...
        LocalDateTime end = start.plusHours(3);
        when(resourceRepository.findAll()).thenReturn(List.of(
                resource(1L, LabType.CHEMISTRY, 3, 3, 5, "70.00"),
                resource(2L, LabType.CHEMISTRY, 5, 4, 5, "90.00")));
    ...
    void shouldRejectWhenTimeWindowIsTooShort() {
        ...
        assertThrows(RuntimeException.class,
                () -> service.findBestResource(LabType.CHEMISTRY,
                        3, 3, 20, 1, start, end));
    }

    @Test
    void shouldCalculateStartedRequiredHoursAndPrice() {
        SchoolLab resource = resource(1L, LabType.CHEMISTRY,
                3, 3, 5, "100.00");
    ...
    private SchoolLab resource(Long id, LabType category, int capacity,
            int quality, int unitsPerHour, String price) {
        SchoolLab resource = new SchoolLab();
        resource.setId(id);
