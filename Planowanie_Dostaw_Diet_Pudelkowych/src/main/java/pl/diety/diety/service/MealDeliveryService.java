package pl.diety.diety.service;
...
public class MealDeliveryService {
    private final MealRouteRepository resourceRepository;
    private final MealDeliveryRepository recordRepository;
    private final AppUserService appUserService;

    public List<MealRoute> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<MealDelivery> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<MealDelivery> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public MealRoute findBestResource(DietArea category, int requiredCapacity,
    ...
    @Transactional
    public MealDelivery createRecord(DietArea category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
        MealRoute resource = findBestResource(category, requiredCapacity, minQuality,
                taskUnits, priority, startTime, endTime);
        MealDelivery record = new MealDelivery();
    ...
    public BigDecimal calculateScore(MealRoute resource, int requiredCapacity,
    ...
    public BigDecimal calculatePrice(MealRoute resource, int taskUnits, int priority) {
    ...
    public int calculateRequiredHours(MealRoute resource, int taskUnits) {
    ...
    private void validateSearch(DietArea category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
