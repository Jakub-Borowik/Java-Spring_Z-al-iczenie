...
import pl.okna.okna.repository.WindowCleaningOrderRepository;
import pl.okna.okna.repository.WindowCleaningTeamRepository;

@Service
@RequiredArgsConstructor
public class WindowCleaningOrderService {
    private final WindowCleaningTeamRepository resourceRepository;
    private final WindowCleaningOrderRepository recordRepository;
    private final AppUserService appUserService;

    public List<WindowCleaningTeam> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<WindowCleaningOrder> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<WindowCleaningOrder> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public WindowCleaningTeam findBestResource(WindowServiceType category, int requiredCapacity,
        ...
        return resourceRepository.findAll().stream()
                .filter(WindowCleaningTeam::isAvailable)
    ...
    @Transactional
    public WindowCleaningOrder createRecord(WindowServiceType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
        WindowCleaningTeam resource = findBestResource(category, requiredCapacity, minQuality,
                taskUnits, priority, startTime, endTime);
        WindowCleaningOrder record = new WindowCleaningOrder();
    ...
    public BigDecimal calculateScore(WindowCleaningTeam resource, int requiredCapacity,
    ...
    public BigDecimal calculatePrice(WindowCleaningTeam resource, int taskUnits, int priority) {
    ...
    public int calculateRequiredHours(WindowCleaningTeam resource, int taskUnits) {
    ...
    private void validateSearch(WindowServiceType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
