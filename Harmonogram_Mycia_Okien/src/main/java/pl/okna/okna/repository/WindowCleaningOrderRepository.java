...
public interface WindowCleaningOrderRepository extends JpaRepository<WindowCleaningOrder, Long> {
    List<WindowCleaningOrder> findByUserId(Long userId);

    @Query("""
            select count(record) > 0 from WindowCleaningOrder record
