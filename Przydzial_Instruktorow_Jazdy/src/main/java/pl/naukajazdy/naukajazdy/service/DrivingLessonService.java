package pl.naukajazdy.naukajazdy.service;
...
public class DrivingLessonService {
    private final DrivingInstructorRepository resourceRepository;
    private final DrivingLessonRepository recordRepository;
    private final AppUserService appUserService;

    public List<DrivingInstructor> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<DrivingLesson> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<DrivingLesson> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public DrivingInstructor findBestResource(LessonType category, int requiredCapacity,
        ...
        return resourceRepository.findAll().stream()
                .filter(DrivingInstructor::isAvailable)
    ...
    @Transactional
    public DrivingLesson createRecord(LessonType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
        DrivingInstructor resource = findBestResource(category, requiredCapacity, minQuality,
                taskUnits, priority, startTime, endTime);
        DrivingLesson record = new DrivingLesson();
    ...
    public BigDecimal calculateScore(DrivingInstructor resource, int requiredCapacity,
    ...
    public BigDecimal calculatePrice(DrivingInstructor resource, int taskUnits, int priority) {
    ...
    public int calculateRequiredHours(DrivingInstructor resource, int taskUnits) {
    ...
    private void validateSearch(LessonType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
