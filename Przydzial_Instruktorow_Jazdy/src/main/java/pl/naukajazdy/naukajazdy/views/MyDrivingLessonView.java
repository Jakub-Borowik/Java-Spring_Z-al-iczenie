package pl.naukajazdy.naukajazdy.views;

import com.vaadin.flow.component.grid.Grid;
import com.vaadin.flow.component.html.*;
import com.vaadin.flow.component.orderedlayout.VerticalLayout;
import com.vaadin.flow.router.*;
import jakarta.annotation.security.RolesAllowed;
import pl.naukajazdy.naukajazdy.model.DrivingLesson;
import pl.naukajazdy.naukajazdy.service.DrivingLessonService;

@Route("driving-lessons/my")
@PageTitle("Moje wpisy")
@RolesAllowed("CLIENT")
public class MyDrivingLessonView extends VerticalLayout {
    public MyDrivingLessonView(DrivingLessonService service) {
        Grid<DrivingLesson> grid = new Grid<>(DrivingLesson.class, false);
        grid.addColumn(record -> record.getResource().getName()).setHeader("Instruktor");
        grid.addColumn(DrivingLesson::getCategory).setHeader("Kategoria");
        grid.addColumn(DrivingLesson::getRequiredCapacity).setHeader("Pojemnosc");
        grid.addColumn(DrivingLesson::getMinQuality).setHeader("Jakosc");
        grid.addColumn(DrivingLesson::getTaskUnits).setHeader("Jednostki");
        grid.addColumn(DrivingLesson::getCalculatedHours).setHeader("Godziny");
        grid.addColumn(DrivingLesson::getScore).setHeader("Wynik");
        grid.addColumn(DrivingLesson::getTotalPrice).setHeader("Koszt");
        grid.setItems(service.getMine());
        add(new Anchor("driving-lessons", "Panel klienta"), new H2("Moje wpisy"), grid);
    }
}
