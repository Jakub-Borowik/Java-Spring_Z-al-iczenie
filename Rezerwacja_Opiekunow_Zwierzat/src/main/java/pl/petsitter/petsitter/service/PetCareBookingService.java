package pl.petsitter.petsitter.service;
...
import pl.petsitter.petsitter.repository.PetCareBookingRepository;
import pl.petsitter.petsitter.repository.PetSitterRepository;

@Service
@RequiredArgsConstructor
public class PetCareBookingService {
    private final PetSitterRepository resourceRepository;
    private final PetCareBookingRepository recordRepository;
    private final AppUserService appUserService;

    public List<PetSitter> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<PetCareBooking> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<PetCareBooking> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public PetSitter findBestResource(PetCareType category, int requiredCapacity,
    ...
    @Transactional
    public PetCareBooking createRecord(PetCareType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
        PetSitter resource = findBestResource(category, requiredCapacity, minQuality,
                taskUnits, priority, startTime, endTime);
        PetCareBooking record = new PetCareBooking();
    ...
    public BigDecimal calculateScore(PetSitter resource, int requiredCapacity,
    ...
    public BigDecimal calculatePrice(PetSitter resource, int taskUnits, int priority) {
    ...
    public int calculateRequiredHours(PetSitter resource, int taskUnits) {
    ...
    private void validateSearch(PetCareType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
