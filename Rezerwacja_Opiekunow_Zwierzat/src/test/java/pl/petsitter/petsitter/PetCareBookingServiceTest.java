package pl.petsitter.petsitter;
...
import pl.petsitter.petsitter.service.AppUserService;
import pl.petsitter.petsitter.service.PetCareBookingService;

@ExtendWith(MockitoExtension.class)
class PetCareBookingServiceTest {
    @Mock
    private PetSitterRepository resourceRepository;

    @Mock
    private PetCareBookingRepository recordRepository;

    @Mock
    private AppUserService appUserService;

    @InjectMocks
    private PetCareBookingService service;
    ...
    void shouldChooseBestScoredResource() {
        ...
        when(resourceRepository.findAll()).thenReturn(List.of(
                resource(1L, PetCareType.DOG, 10, 5, 5, "100.00"),
                resource(2L, PetCareType.DOG, 3, 3, 5, "70.00"),
                resource(3L, PetCareType.CAT, 3, 3, 5, "50.00")));
        when(recordRepository.existsOverlapping(anyLong(), eq(start), eq(end))).thenReturn(false);

        PetSitter result = service.findBestResource(PetCareType.DOG,
                3, 3, 10, 1, start, end);
    ...
    void shouldSkipOverlappingBetterResource() {
        ...
        LocalDateTime end = start.plusHours(3);
        when(resourceRepository.findAll()).thenReturn(List.of(
                resource(1L, PetCareType.DOG, 3, 3, 5, "70.00"),
                resource(2L, PetCareType.DOG, 5, 4, 5, "90.00")));
    ...
    void shouldRejectWhenTimeWindowIsTooShort() {
        ...
        assertThrows(RuntimeException.class,
                () -> service.findBestResource(PetCareType.DOG,
                        3, 3, 20, 1, start, end));
    }

    @Test
    void shouldCalculateStartedRequiredHoursAndPrice() {
        PetSitter resource = resource(1L, PetCareType.DOG,
                3, 3, 5, "100.00");
    ...
    private PetSitter resource(Long id, PetCareType category, int capacity,
            int quality, int unitsPerHour, String price) {
        PetSitter resource = new PetSitter();
        resource.setId(id);
