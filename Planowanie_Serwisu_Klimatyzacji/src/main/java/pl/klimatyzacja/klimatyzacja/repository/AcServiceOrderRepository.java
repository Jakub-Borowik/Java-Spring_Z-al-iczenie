...
public interface AcServiceOrderRepository extends JpaRepository<AcServiceOrder, Long> {
    List<AcServiceOrder> findByUserId(Long userId);

    @Query("""
            select count(record) > 0 from AcServiceOrder record
