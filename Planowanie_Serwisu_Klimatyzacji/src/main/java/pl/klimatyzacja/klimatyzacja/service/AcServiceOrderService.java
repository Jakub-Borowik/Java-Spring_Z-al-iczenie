package pl.klimatyzacja.klimatyzacja.service;
...
public class AcServiceOrderService {
    private final AcTechnicianRepository resourceRepository;
    private final AcServiceOrderRepository recordRepository;
    private final AppUserService appUserService;

    public List<AcTechnician> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<AcServiceOrder> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<AcServiceOrder> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public AcTechnician findBestResource(AcServiceType category, int requiredCapacity,
        ...
        return resourceRepository.findAll().stream()
                .filter(AcTechnician::isAvailable)
    ...
    @Transactional
    public AcServiceOrder createRecord(AcServiceType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
        AcTechnician resource = findBestResource(category, requiredCapacity, minQuality,
                taskUnits, priority, startTime, endTime);
        AcServiceOrder record = new AcServiceOrder();
    ...
    public BigDecimal calculateScore(AcTechnician resource, int requiredCapacity,
    ...
    public BigDecimal calculatePrice(AcTechnician resource, int taskUnits, int priority) {
    ...
    public int calculateRequiredHours(AcTechnician resource, int taskUnits) {
    ...
    private void validateSearch(AcServiceType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
