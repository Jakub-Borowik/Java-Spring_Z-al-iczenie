package pl.remonty.remonty.service;
...
public class RenovationOrderService {
    private final RenovationCrewRepository resourceRepository;
    private final RenovationOrderRepository recordRepository;
    private final AppUserService appUserService;

    public List<RenovationCrew> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<RenovationOrder> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<RenovationOrder> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public RenovationCrew findBestResource(RenovationType category, int requiredCapacity,
        ...
        return resourceRepository.findAll().stream()
                .filter(RenovationCrew::isAvailable)
    ...
    @Transactional
    public RenovationOrder createRecord(RenovationType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
        RenovationCrew resource = findBestResource(category, requiredCapacity, minQuality,
                taskUnits, priority, startTime, endTime);
        RenovationOrder record = new RenovationOrder();
    ...
    public BigDecimal calculateScore(RenovationCrew resource, int requiredCapacity,
    ...
    public BigDecimal calculatePrice(RenovationCrew resource, int taskUnits, int priority) {
    ...
    public int calculateRequiredHours(RenovationCrew resource, int taskUnits) {
    ...
    private void validateSearch(RenovationType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
