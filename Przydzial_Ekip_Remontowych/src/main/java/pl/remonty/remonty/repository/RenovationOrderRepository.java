...
public interface RenovationOrderRepository extends JpaRepository<RenovationOrder, Long> {
    List<RenovationOrder> findByUserId(Long userId);

    @Query("""
            select count(record) > 0 from RenovationOrder record
