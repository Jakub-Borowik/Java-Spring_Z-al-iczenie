package pl.parking.parking.service;
...
import pl.parking.parking.repository.ParkingReservationRepository;
import pl.parking.parking.repository.ParkingSpotRepository;

@Service
@RequiredArgsConstructor
public class ParkingReservationService {
    private final ParkingSpotRepository resourceRepository;
    private final ParkingReservationRepository recordRepository;
    private final AppUserService appUserService;

    public List<ParkingSpot> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<ParkingReservation> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<ParkingReservation> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public ParkingSpot findBestResource(ParkingType category, int requiredCapacity,
    ...
    @Transactional
    public ParkingReservation createRecord(ParkingType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
        ParkingSpot resource = findBestResource(category, requiredCapacity, minQuality,
                taskUnits, priority, startTime, endTime);
        ParkingReservation record = new ParkingReservation();
    ...
    public BigDecimal calculateScore(ParkingSpot resource, int requiredCapacity,
    ...
    public BigDecimal calculatePrice(ParkingSpot resource, int taskUnits, int priority) {
    ...
    public int calculateRequiredHours(ParkingSpot resource, int taskUnits) {
    ...
    private void validateSearch(ParkingType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
