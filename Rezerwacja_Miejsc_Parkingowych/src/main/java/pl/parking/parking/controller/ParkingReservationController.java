package pl.parking.parking.controller;

import java.time.LocalDateTime;
import java.util.List;
import org.springframework.format.annotation.DateTimeFormat;
import org.springframework.web.bind.annotation.*;
import lombok.RequiredArgsConstructor;
import pl.parking.parking.model.ParkingType;
import pl.parking.parking.model.ParkingReservation;
import pl.parking.parking.service.ParkingReservationService;

@RestController
@RequiredArgsConstructor
@RequestMapping("/api/parking")
public class ParkingReservationController {
    private final ParkingReservationService service;

    @PostMapping
    public String create(@RequestParam ParkingType category,
            @RequestParam int requiredCapacity,
            @RequestParam int minQuality,
            @RequestParam int taskUnits,
            @RequestParam int priority,
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime startTime,
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime endTime) {
        ParkingReservation record = service.createRecord(category, requiredCapacity, minQuality,
                taskUnits, priority, startTime, endTime);
        return "Created id: " + record.getId() + ", price: " + record.getTotalPrice()
                + ", score: " + record.getScore();
    }

    @GetMapping("/all")
    public List<ParkingReservation> all() {
        return service.getAllRecords();
    }
}
