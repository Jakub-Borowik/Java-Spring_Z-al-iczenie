...
import pl.parking.parking.service.AppUserService;
import pl.parking.parking.service.ParkingReservationService;

@ExtendWith(MockitoExtension.class)
class ParkingReservationServiceTest {
    @Mock
    private ParkingSpotRepository resourceRepository;

    @Mock
    private ParkingReservationRepository recordRepository;
    ...
    void shouldChooseBestScoredResource() {
        ...
        when(resourceRepository.findAll()).thenReturn(List.of(
                resource(1L, ParkingType.STANDARD, 10, 5, 5, "100.00"),
                resource(2L, ParkingType.STANDARD, 3, 3, 5, "70.00"),
                resource(3L, ParkingType.COVERED, 3, 3, 5, "50.00")));
        when(recordRepository.existsOverlapping(anyLong(), eq(start), eq(end))).thenReturn(false);

        ParkingSpot result = service.findBestResource(ParkingType.STANDARD,
                3, 3, 10, 1, start, end);
    ...
    void shouldSkipOverlappingBetterResource() {
        ...
        LocalDateTime end = start.plusHours(3);
        when(resourceRepository.findAll()).thenReturn(List.of(
                resource(1L, ParkingType.STANDARD, 3, 3, 5, "70.00"),
                resource(2L, ParkingType.STANDARD, 5, 4, 5, "90.00")));
    ...
    void shouldRejectWhenTimeWindowIsTooShort() {
        ...
        assertThrows(RuntimeException.class,
                () -> service.findBestResource(ParkingType.STANDARD,
                        3, 3, 20, 1, start, end));
    }

    @Test
    void shouldCalculateStartedRequiredHoursAndPrice() {
        ParkingSpot resource = resource(1L, ParkingType.STANDARD,
                3, 3, 5, "100.00");
    ...
    private ParkingSpot resource(Long id, ParkingType category, int capacity,
            int quality, int unitsPerHour, String price) {
        ParkingSpot resource = new ParkingSpot();
        resource.setId(id);
