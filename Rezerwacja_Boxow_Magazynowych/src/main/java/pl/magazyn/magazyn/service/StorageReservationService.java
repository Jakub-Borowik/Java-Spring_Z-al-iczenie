package pl.magazyn.magazyn.service;
...
public class StorageReservationService {
    private final StorageBoxRepository resourceRepository;
    private final StorageReservationRepository recordRepository;
    private final AppUserService appUserService;

    public List<StorageBox> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<StorageReservation> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<StorageReservation> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public StorageBox findBestResource(StorageType category, int requiredCapacity,
    ...
    @Transactional
    public StorageReservation createRecord(StorageType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
        StorageBox resource = findBestResource(category, requiredCapacity, minQuality,
                taskUnits, priority, startTime, endTime);
        StorageReservation record = new StorageReservation();
    ...
    public BigDecimal calculateScore(StorageBox resource, int requiredCapacity,
    ...
    public BigDecimal calculatePrice(StorageBox resource, int taskUnits, int priority) {
    ...
    public int calculateRequiredHours(StorageBox resource, int taskUnits) {
    ...
    private void validateSearch(StorageType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
