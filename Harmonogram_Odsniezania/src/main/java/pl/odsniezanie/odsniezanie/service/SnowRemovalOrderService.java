package pl.odsniezanie.odsniezanie.service;
...
import pl.odsniezanie.odsniezanie.repository.SnowRemovalOrderRepository;
import pl.odsniezanie.odsniezanie.repository.SnowRemovalVehicleRepository;

@Service
@RequiredArgsConstructor
public class SnowRemovalOrderService {
    private final SnowRemovalVehicleRepository resourceRepository;
    private final SnowRemovalOrderRepository recordRepository;
    private final AppUserService appUserService;

    public List<SnowRemovalVehicle> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<SnowRemovalOrder> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<SnowRemovalOrder> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public SnowRemovalVehicle findBestResource(SnowZone category, int requiredCapacity,
        ...
        return resourceRepository.findAll().stream()
                .filter(SnowRemovalVehicle::isAvailable)
    ...
    @Transactional
    public SnowRemovalOrder createRecord(SnowZone category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
        SnowRemovalVehicle resource = findBestResource(category, requiredCapacity, minQuality,
                taskUnits, priority, startTime, endTime);
        SnowRemovalOrder record = new SnowRemovalOrder();
    ...
    public BigDecimal calculateScore(SnowRemovalVehicle resource, int requiredCapacity,
    ...
    public BigDecimal calculatePrice(SnowRemovalVehicle resource, int taskUnits, int priority) {
    ...
    public int calculateRequiredHours(SnowRemovalVehicle resource, int taskUnits) {
    ...
    private void validateSearch(SnowZone category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
