...
public interface InternetInstallationRepository extends JpaRepository<InternetInstallation, Long> {
    List<InternetInstallation> findByUserId(Long userId);

    @Query("""
            select count(record) > 0 from InternetInstallation record
