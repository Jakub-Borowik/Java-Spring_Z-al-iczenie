package pl.internet.internet.service;
...
public class InternetInstallationService {
    private final InternetInstallerRepository resourceRepository;
    private final InternetInstallationRepository recordRepository;
    private final AppUserService appUserService;

    public List<InternetInstaller> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<InternetInstallation> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<InternetInstallation> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public InternetInstaller findBestResource(InstallationType category, int requiredCapacity,
        ...
        return resourceRepository.findAll().stream()
                .filter(InternetInstaller::isAvailable)
    ...
    @Transactional
    public InternetInstallation createRecord(InstallationType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
        InternetInstaller resource = findBestResource(category, requiredCapacity, minQuality,
                taskUnits, priority, startTime, endTime);
        InternetInstallation record = new InternetInstallation();
    ...
    public BigDecimal calculateScore(InternetInstaller resource, int requiredCapacity,
    ...
    public BigDecimal calculatePrice(InternetInstaller resource, int taskUnits, int priority) {
    ...
    public int calculateRequiredHours(InternetInstaller resource, int taskUnits) {
    ...
    private void validateSearch(InstallationType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
