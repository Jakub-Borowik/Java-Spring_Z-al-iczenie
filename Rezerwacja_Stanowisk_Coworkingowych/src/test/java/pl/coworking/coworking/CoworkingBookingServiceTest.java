package pl.coworking.coworking;
...
import pl.coworking.coworking.service.AppUserService;
import pl.coworking.coworking.service.CoworkingBookingService;

@ExtendWith(MockitoExtension.class)
class CoworkingBookingServiceTest {
    @Mock
    private WorkDeskRepository resourceRepository;

    @Mock
    private CoworkingBookingRepository recordRepository;

    @Mock
    private AppUserService appUserService;

    @InjectMocks
    private CoworkingBookingService service;
    ...
    void shouldChooseBestScoredResource() {
        ...
        when(resourceRepository.findAll()).thenReturn(List.of(
                resource(1L, DeskType.OPEN_SPACE, 10, 5, 5, "100.00"),
                resource(2L, DeskType.OPEN_SPACE, 3, 3, 5, "70.00"),
                resource(3L, DeskType.QUIET, 3, 3, 5, "50.00")));
        when(recordRepository.existsOverlapping(anyLong(), eq(start), eq(end))).thenReturn(false);

        WorkDesk result = service.findBestResource(DeskType.OPEN_SPACE,
                3, 3, 10, 1, start, end);
    ...
    void shouldSkipOverlappingBetterResource() {
        ...
        LocalDateTime end = start.plusHours(3);
        when(resourceRepository.findAll()).thenReturn(List.of(
                resource(1L, DeskType.OPEN_SPACE, 3, 3, 5, "70.00"),
                resource(2L, DeskType.OPEN_SPACE, 5, 4, 5, "90.00")));
    ...
    void shouldRejectWhenTimeWindowIsTooShort() {
        ...
        assertThrows(RuntimeException.class,
                () -> service.findBestResource(DeskType.OPEN_SPACE,
                        3, 3, 20, 1, start, end));
    }

    @Test
    void shouldCalculateStartedRequiredHoursAndPrice() {
        WorkDesk resource = resource(1L, DeskType.OPEN_SPACE,
                3, 3, 5, "100.00");
    ...
    private WorkDesk resource(Long id, DeskType category, int capacity,
            int quality, int unitsPerHour, String price) {
        WorkDesk resource = new WorkDesk();
        resource.setId(id);
