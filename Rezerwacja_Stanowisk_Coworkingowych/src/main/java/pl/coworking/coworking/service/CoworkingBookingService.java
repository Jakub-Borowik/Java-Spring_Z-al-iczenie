package pl.coworking.coworking.service;
...
import pl.coworking.coworking.repository.CoworkingBookingRepository;
import pl.coworking.coworking.repository.WorkDeskRepository;

@Service
@RequiredArgsConstructor
public class CoworkingBookingService {
    private final WorkDeskRepository resourceRepository;
    private final CoworkingBookingRepository recordRepository;
    private final AppUserService appUserService;

    public List<WorkDesk> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<CoworkingBooking> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<CoworkingBooking> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public WorkDesk findBestResource(DeskType category, int requiredCapacity,
    ...
    @Transactional
    public CoworkingBooking createRecord(DeskType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
        WorkDesk resource = findBestResource(category, requiredCapacity, minQuality,
                taskUnits, priority, startTime, endTime);
        CoworkingBooking record = new CoworkingBooking();
    ...
    public BigDecimal calculateScore(WorkDesk resource, int requiredCapacity,
    ...
    public BigDecimal calculatePrice(WorkDesk resource, int taskUnits, int priority) {
    ...
    public int calculateRequiredHours(WorkDesk resource, int taskUnits) {
    ...
    private void validateSearch(DeskType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
