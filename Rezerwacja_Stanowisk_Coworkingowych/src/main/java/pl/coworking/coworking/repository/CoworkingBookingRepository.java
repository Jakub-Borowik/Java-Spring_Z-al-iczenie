...
public interface CoworkingBookingRepository extends JpaRepository<CoworkingBooking, Long> {
    List<CoworkingBooking> findByUserId(Long userId);

    @Query("""
            select count(record) > 0 from CoworkingBooking record
