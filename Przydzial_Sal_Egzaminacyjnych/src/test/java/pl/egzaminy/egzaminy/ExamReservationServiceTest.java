...
import pl.egzaminy.egzaminy.service.AppUserService;
import pl.egzaminy.egzaminy.service.ExamReservationService;

@ExtendWith(MockitoExtension.class)
class ExamReservationServiceTest {
    @Mock
    private ExamRoomRepository resourceRepository;

    @Mock
    private ExamReservationRepository recordRepository;

    @Mock
    private AppUserService appUserService;

    @InjectMocks
    private ExamReservationService service;
    ...
    void shouldChooseBestScoredResource() {
        ...
        when(resourceRepository.findAll()).thenReturn(List.of(
                resource(1L, ExamType.WRITTEN, 10, 5, 5, "100.00"),
                resource(2L, ExamType.WRITTEN, 3, 3, 5, "70.00"),
                resource(3L, ExamType.COMPUTER, 3, 3, 5, "50.00")));
        when(recordRepository.existsOverlapping(anyLong(), eq(start), eq(end))).thenReturn(false);

        ExamRoom result = service.findBestResource(ExamType.WRITTEN,
                3, 3, 10, 1, start, end);
    ...
    void shouldSkipOverlappingBetterResource() {
        ...
        LocalDateTime end = start.plusHours(3);
        when(resourceRepository.findAll()).thenReturn(List.of(
                resource(1L, ExamType.WRITTEN, 3, 3, 5, "70.00"),
                resource(2L, ExamType.WRITTEN, 5, 4, 5, "90.00")));
    ...
    void shouldRejectWhenTimeWindowIsTooShort() {
        ...
        assertThrows(RuntimeException.class,
                () -> service.findBestResource(ExamType.WRITTEN,
                        3, 3, 20, 1, start, end));
    }

    @Test
    void shouldCalculateStartedRequiredHoursAndPrice() {
        ExamRoom resource = resource(1L, ExamType.WRITTEN,
                3, 3, 5, "100.00");
    ...
    private ExamRoom resource(Long id, ExamType category, int capacity,
            int quality, int unitsPerHour, String price) {
        ExamRoom resource = new ExamRoom();
        resource.setId(id);
