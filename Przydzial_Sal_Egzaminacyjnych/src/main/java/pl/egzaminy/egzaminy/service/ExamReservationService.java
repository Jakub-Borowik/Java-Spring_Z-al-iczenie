package pl.egzaminy.egzaminy.service;
...
import pl.egzaminy.egzaminy.repository.ExamReservationRepository;
import pl.egzaminy.egzaminy.repository.ExamRoomRepository;

@Service
@RequiredArgsConstructor
public class ExamReservationService {
    private final ExamRoomRepository resourceRepository;
    private final ExamReservationRepository recordRepository;
    private final AppUserService appUserService;

    public List<ExamRoom> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<ExamReservation> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<ExamReservation> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public ExamRoom findBestResource(ExamType category, int requiredCapacity,
    ...
    @Transactional
    public ExamReservation createRecord(ExamType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
        ExamRoom resource = findBestResource(category, requiredCapacity, minQuality,
                taskUnits, priority, startTime, endTime);
        ExamReservation record = new ExamReservation();
    ...
    public BigDecimal calculateScore(ExamRoom resource, int requiredCapacity,
    ...
    public BigDecimal calculatePrice(ExamRoom resource, int taskUnits, int priority) {
    ...
    public int calculateRequiredHours(ExamRoom resource, int taskUnits) {
    ...
    private void validateSearch(ExamType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
